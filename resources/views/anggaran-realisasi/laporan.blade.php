                    ...
                    @foreach ($detail_jenis_anggaran->groupBy('jenis_anggaran_id') as $key => $jenis_anggaran)
                        ...
                        @if ($key == 5)
                            <tr class="bg-primary text-white font-weight-bold ">
                                ...
                                <td class="text-right">Rp. {{ substr(number_format($pendapatan_anggaran - $belanja_anggaran, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format($pendapatan_realisasi - $belanja_realisasi, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format(($pendapatan_anggaran - $belanja_anggaran) - ($pendapatan_realisasi - $belanja_realisasi), 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">
                                    @php
                                        try {
                                            $persen = (($pendapatan_realisasi - $belanja_realisasi)/($pendapatan_anggaran - $belanja_anggaran)) * 100;
                                            echo number_format((float)$persen, 2, '.', '') . "%";
                                        } catch (\Throwable $th) {
                                            echo "-";
                                        }
                        ...
                        @elseif($key == 6)
                            <tr class="bg-primary text-white font-weight-bold ">
                                ...
                                <td class="text-right">Rp. {{ substr(number_format($pembiayaan_netto_anggaran, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format($pembiayaan_netto_realisasi, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format($pembiayaan_netto_anggaran - $pembiayaan_netto_realisasi, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">
                                    @php
                                        try {
                                            $persen = ($pembiayaan_netto_realisasi/$pembiayaan_netto_anggaran) * 100;
                                            echo number_format((float)$persen, 2, '.', '') . "%";
                                        } catch (\Throwable $th) {
                                            echo "-";
                                        }
                            ...
                            <tr class="bg-primary text-white font-weight-bold ">
                                ...
                                <td class="text-right">Rp. {{ substr(number_format($anggaran_jenis, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format($pembiayaan_netto_realisasi, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">Rp. {{ substr(number_format($anggaran_jenis - $pembiayaan_netto_realisasi, 2, ',', '.'),0,-3) }}</td>
                                <td class="text-right">
                                    @php
                                        try {
                                            $persen = ($pembiayaan_netto_realisasi/$anggaran_jenis) * 100;
                                            echo number_format((float)$persen, 2, '.', '') . "%";
                                        } catch (\Throwable $th) {
                                            echo "-";
                                        }
